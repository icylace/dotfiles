# rsync
# Autogenerated from man page /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.12.sdk/usr/share/man/man1/rsync.1.gz
...
complete -c rsync -l itemize-changes -l out-format --description 'output (on the client) increases to mention all items that are changed in any…'
...
complete -c rsync -l size-only --description 'Normally rsync will not transfer any files that are already the same size and…'
complete -c rsync -l modify-window --description 'When comparing two timestamps, rsync treats the timestamps as being equal if …'
complete -c rsync -s c -l checksum --description 'This forces the sender to checksum every regular file using a 128-bit MD4 che…'
...
complete -c rsync -l copy-dirlinks --description 'transfer -- see their descriptions for full details).'
...
complete -c rsync -l inplace --description 'This causes rsync not to create a new copy of the file and then move it into …'
...
complete -c rsync -l keep-dirlinks --description 'This option causes the receiving side to treat a symlink to a directory as th…'
complete -c rsync -s H -l hard-links --description 'This tells rsync to look for hard-linked files in the transfer and link toget…'
...
complete -c rsync -l executability --description 'This option causes rsync to preserve the executability (or non-executability)…'
...
complete -c rsync -s S --description 'Try to handle sparse files efficiently so they take up less space on the dest…'
complete -c rsync -s n -l dry-run --description 'This tells rsync to not do any file transfers, instead it will just report th…'
complete -c rsync -s W -l whole-file --description 'With this option the incremental rsync algorithm is not used and the whole fi…'
...
complete -c rsync -l delete-before --description '.'
complete -c rsync -l delete-during -l 'delete-after.' --description '.'
...
complete -c rsync -l 'delete-excluded.' --description 'See --delete (which is implied) for more details on file-deletion.'
complete -c rsync -l ignore-errors --description 'Tells --delete to go ahead and delete files even when there are I/O errors.'
complete -c rsync -l max-delete --description 'This tells rsync not to delete more than NUM files or directories (NUM must b…'
...
complete -c rsync -s B -l block-size --description 'This forces the block size used in the rsync algorithm to a fixed value.'
...
complete -c rsync -s y -l fuzzy --description 'This option tells rsync that it should look for a basis file for any destinat…'
...
complete -c rsync -s i --description 'Requests a simple itemized list of the changes that are being made to each fi…'
...
complete -c rsync -l delay-updates --description 'For the purposes of the daemon-config\'s "refuse options" setting,.'
complete -c rsync -l 'partial.' --description 'refusal of the --partial option can be used to disallow the overwriting of de…'
complete -c rsync -s m -l prune-empty-dirs --description 'This option tells the receiving rsync to get rid of empty directories from th…'
...
complete -c rsync -l password-file --description 'This option allows you to provide a password in a file for accessing a remote…'
complete -c rsync -l list-only --description 'This option will cause the source files to be listed instead of transferred.'
complete -c rsync -l bwlimit --description 'This option allows you to specify a maximum transfer rate in kilobytes per se…'
...
complete -c rsync -s E -l extended-attributes --description 'Apple specific option to copy extended attributes, resource forks, and ACLs.'
complete -c rsync -l cache --description 'Apple specific option to enable filesystem caching of rsync file i/o Otherwis…'
