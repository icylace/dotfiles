...
echo '\tEnable Dark Mode.  Yosemite.'
defaults write -g AppleInterfaceStyle -string 'Dark'

echo '\tEnable full keyboard access (tab through all GUI buttons and fields, not just text boxes and lists).'
defaults write -g AppleKeyboardUIMode -int 3

echo '\tDo not minimize the window when the title bar is double-clicked.'
...
# echo '\tActivate special keyboard behavior with fn + funciton key instead of function key alone.'
...
#
# For all applications, change tabs with cmd-opt-arrow.
...
# echo '\tIncreasing trackpad tracking to 3.'
